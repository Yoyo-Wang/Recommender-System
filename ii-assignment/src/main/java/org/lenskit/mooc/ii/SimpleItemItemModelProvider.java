...
public class SimpleItemItemModelProvider implements Provider<SimpleItemItemModel> {
    ...
    public SimpleItemItemModel get() {
        ...
        // use two for loop to compute items similarities
        Long2DoubleMap euclideanNorm = new Long2DoubleOpenHashMap();
        for (Map.Entry<Long, Long2DoubleMap> entry : itemVectors.entrySet()) {
            Long key = entry.getKey();
            Long2DoubleMap value = entry.getValue();
            Double score = Vectors.euclideanNorm(value);
            euclideanNorm.put(key, score);
        }
        for (Map.Entry<Long, Long2DoubleMap> item1 : itemVectors.entrySet()) {
            Long2DoubleMap map = new Long2DoubleOpenHashMap();
            Long key1 = item1.getKey();
            for (Map.Entry<Long, Long2DoubleMap> item2 : itemVectors.entrySet()) {
                Long key2 = item2.getKey();
                if (!map.containsKey(key2) && key1 != key2) {
                    Long2DoubleMap map1 = item1.getValue();
                    Long2DoubleMap map2 = item2.getValue();
                    Double score = Vectors.dotProduct(map1, map2);
                    if (score > 0) {
                        score = score / (euclideanNorm.get(key1)*euclideanNorm.get(key2));
                        map.put(key2, score);
                    }
                }
            }
            itemSimilarities.put(key1, map);
        }
