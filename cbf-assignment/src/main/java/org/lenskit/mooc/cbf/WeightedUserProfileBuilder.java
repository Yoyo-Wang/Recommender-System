...
public class WeightedUserProfileBuilder implements UserProfileBuilder {
    ...
    public Map<String, Double> makeUserProfile(@Nonnull List<Rating> ratings) {
        ...
        for (Rating r: ratings) {
            Map<String, Double> map = model.getItemVector(r.getItemId());
            for (Map.Entry<String, Double> entry : map.entrySet()) {
                String key = entry.getKey();
                double rui = r.getValue();
                if (rui >= 0.0) {
                    double score = (rui - mean) * entry.getValue();
                    if (profile.containsKey(key)) {
                        Double res = profile.get(key) + score;
                        profile.put(key, res);
                    } else {
                        profile.put(key, score);
                    }
                }
            }
