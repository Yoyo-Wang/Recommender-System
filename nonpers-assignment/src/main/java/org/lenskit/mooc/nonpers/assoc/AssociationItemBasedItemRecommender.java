...
public class AssociationItemBasedItemRecommender extends AbstractItemBasedItemRecommender {
    ...
    private ResultList recommendItems(int n, long refItem, LongSet candidates) {
        ...
        TreeSet<Result> ts = new TreeSet<Result>(new AssociationItemBasedItemRecommender.MyComp());
        for (long id : candidates) {
            Result result = Results.create(id, model.getItemAssociation(refItem, id));
            ts.add(result);
        }
        int count = 0;
        for (Result rs : ts) {
